...
function checkTokenIsAvailable() {
    ...
    try {
        ...
        if (token == "") {
            if (language == "EN")
                SpreadsheetApp.getUi().alert(
                    "You deleted access token, please get token to use the device"
                );
            else
                SpreadsheetApp.getUi().alert(
                    "Bạn đã xoá Access Token, vui lòng lấy Token để sử dụng dịch vụ"
                );
            showGetToken();
            return false;
        }
...
function getLanguage() {
    if (SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Values of API"))
        ...
        SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Các giá trị API")
    )
        return "VN";
    return "";
}
